...
function Skills() {
  ...
  const container = {
    ...
    show: {
      opacity: 1,
      transition: {
        staggerChildren: 0.3, // Increase the delay between each skill
      },
    },
  };

  const item = {
    hidden: { y: 100, opacity: 0 }, // Start from lower down
    show: {
      y: 0,
      opacity: 1,
      transition: { duration: 1, ease: "easeOut" }, // Slow down the duration
    },
  };
